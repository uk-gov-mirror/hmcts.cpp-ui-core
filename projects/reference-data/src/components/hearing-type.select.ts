...
import { getHearingTypes, ReferenceDataState } from '../reducers/index';
import { HearingType } from '../reference-data.interfaces';
import { AsyncPipe } from '@angular/common';

@Component({
  selector: 'cpp-hearing-type-select',
  template: `
    <pdk-select
  ...
  providers: [
    {
      ...
      useExisting: forwardRef(() => HearingTypeSelectComponent),
      multi: true
    },
    {
...
export class HearingTypeSelectComponent implements OnInit, ControlValueAccessor, FormFieldControl {
  ...
  constructor(private store: Store<ReferenceDataState>) {
    this.options$ = this.store.pipe(
      ...
      map((hearingTypes) => {
        if (hearingTypes) {
          return hearingTypes.map((hearingType) => ({
  ...
  ngOnInit() {
    if (this.fetchOptionsOnMount) {
      this.store.pipe(select(getHearingTypes), take(1)).subscribe((hearingTypes) => {
        if (!hearingTypes) {
  ...
  registerOnChange = (fn: (_: any) => void): void => {
    this.selectRef.registerOnChange((hearingTypeId?: string) => {
      this.store
        .pipe(
          select(getHearingTypes),
          take(1),
          map((hearingTypes) => {
            if (hearingTypes) {
              return hearingTypes.find(({ id }) => id === hearingTypeId);
            }
            return undefined;
          })
        )
